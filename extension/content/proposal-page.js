...
(function () {
  ...
  // ── Logo ───────────────────────────────────────────────────────────────────
  // Real extension icon (icons/icon128.png) — same source used elsewhere
  // (job-match-toast.js) instead of a hand-drawn SVG standing in for it.
  const LOGO_URL = chrome.runtime.getURL('icons/icon128.png');
  const LOGO_SVG = `<img class="sn-pp-icon" src="${LOGO_URL}" width="42" height="42" alt="">`;

  const LOGO_SM = `<img src="${LOGO_URL}" width="28" height="28" alt="">`;
  ...
  function injectTextareaIcon(clField, jobData) {
    ...
    icon.innerHTML = `
      <img class="sn-cl-svg" src="${LOGO_URL}" width="26" height="26" alt="">
      <span class="sn-email-badge">!</span>
    `;

    container.appendChild(icon);

    // One-time use — once the cover letter has content, the icon disappears
    // and the floating button unlocks (see syncFieldGate). Reappears if the
    // field is cleared, so regenerating from scratch is still possible.
    const syncIconState = () => syncFieldGate(clField);
  ...
  // The floating button and the in-field icon are mutually exclusive, driven
  // by one source of truth (syncFieldGate, below): empty field → in-field
  // icon shown, floating button locked. Field has content (generated by us
  // OR typed/edited by hand) → floating button unlocked, in-field icon
  // hidden. Not tied to a generation event finishing — that left a dead end
  // where the field had content (so the icon hid) but generate() had never
  // actually run, so the floating button never unlocked either.
  function syncFieldGate(clField) {
    const filled = clField.value.trim().length > 0;
    document.getElementById('sn-pp-btn')?.classList.toggle('sn-pp-locked', !filled);
    document.getElementById('sn-cl-icon')?.classList.toggle('sn-cl-filled', filled);
  }
  ...
  function buildUI(jobData) {
    ...
    const clFieldNow = getCLField();
    if (!clFieldNow || !clFieldNow.value.trim()) btn.classList.add('sn-pp-locked');
  ...
  async function generate(jobData, source = 'chatbot') {
    ...
    try {
      ...
      // Button lock/unlock is NOT handled here — vueInject() already fired a
      // real 'input' event when the text landed, which syncFieldGate picked
      // up. This only owns the transient spinner→success-flash animation.
      function markDone() {
        if (isInline) {
          if (clIcon) {
