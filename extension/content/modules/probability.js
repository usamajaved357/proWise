...
window.SnagAI.calcWinProbability = function(jobStats, profile, filters) {
  ...
  // Hyphens/slashes/underscores must become a SPACE, not just get deleted —
  // stripping them outright glues adjacent words together ("AI-Generated"
  // → "aigenerated"), which then fails to match a profile skill written
  // with a space ("AI Generated"). Applied to both sides so it doesn't
  // matter which one uses a hyphen.
  const normSkill = s => s.toLowerCase().replace(/[-_/]/g, ' ').replace(/[^a-z0-9+#.\s]/gi, '').replace(/\s+/g, ' ').trim();

  const rawJobSkills = Array.isArray(jobStats.jobSkills) ? jobStats.jobSkills : [];
  const jobSkillsNorm = rawJobSkills.map(s => normSkill(s)).filter(Boolean);
  ...
  const profileSkillsLower = normSkill(profileSkillsStr);
  ...
  // Already hired
  const _hiredCount = jobStats.hiredCount || 0;
  if (_hiredCount > 0) {
    riskItems.push('Client already {{hired ' + _hiredCount + ' freelancer' + (_hiredCount > 1 ? 's' : '') + '}}, job is likely closed');
  }
  ...
  // Client spent — only evaluated when we actually know the figure. Some
  // clients hide their total spend on Upwork, which left clientTotalSpent
  // null/unparsed and clientSpentNum defaulted to 0 — indistinguishable
